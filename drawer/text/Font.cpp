...
void FC_SetRenderCallback(
        FC_Rect (*callback)(FC_Image *src, FC_Rect *srcrect, FC_Target *dest, float x, float y, float xscale,
                            float yscale)) {
    if (callback == NULL)
...
#endif
{
...

    {
        ...
        for (; *source_string != '\0'; source_string = U8_next(source_string)) {
            ...
            if (packed || FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0), glyph_surf->w,
                                           surfaces[num_surfaces - 1]->w, surfaces[num_surfaces - 1]->h) != NULL) {
...
Uint8 FC_LoadFont_RW(FC_Font *font, FC_Target *renderer, SDL_RWops *file_rwops_ttf, Uint8 own_rwops, Uint32 pointSize,
...
static FC_StringList *FC_GetBufferFitToColumn(FC_Font *font, int width, FC_Scale scale, Uint8 keep_newlines) {
    ...
    for (iter = ls; iter != NULL; iter = iter->next) {
        ...
        if (width > 0 && FC_GetWidth(font, "%s", line) > width) {
            ...
            for (word_iter = words->next, spaces_iter = spaces->next; word_iter != NULL && spaces_iter !=
                                                                                           NULL; word_iter = word_iter->next, spaces_iter = spaces_iter->next) {
...
static void FC_DrawColumnFromBuffer(FC_Font *font, FC_Target *dest, FC_Rect box, int *total_height, FC_Scale scale,
                                    FC_AlignEnum align) {
    int y = box.y;
    FC_StringList *ls, *iter;
